...
/*   Updated: 2018/02/09 13:51:12 by tvisenti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		send_get_server(int fd, struct stat st, void *ptr)
{
	...
	if (recv_alert("SEND", fd) < 1)
		return (print_error_get_put_server("can't get size from client side"));
...
void			cmd_get_server(int fd, char *buf)
{
	...
	if ((file = open_file_rdonly(buf, fd)) == -1)
		return (print_error_get_put_server("open() server side failed"));
	if (recv_alert("WRONLY_OK", fd) < 1)
		return (print_error_get_put_server("open() client side failed"));
	if ((fstat(file, &st)) == -1)
	{
		ft_putendl_fd("TEST_ERROR", fd);
		return (print_error_get_put_server("fstat() server side failed"));
	}
	if ((ptr = mmap(NULL, st.st_size, PROT_READ, MAP_PRIVATE, file, 0))
			== MAP_FAILED)
	{
		ft_putendl_fd("TEST_ERROR", fd);
		return (print_error_get_put_server("mmap() server side failed"));
	}
